...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cardsSection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cards-sections.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="invitee_name">{{ trans('cruds.cardsSection.fields.invitee_name') }}</label>
                <input class="form-control {{ $errors->has('invitee_name') ? 'is-invalid' : '' }}" type="text" name="invitee_name" id="invitee_name" value="{{ old('invitee_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('persons_invited') ? 'is-invalid' : '' }}" type="number" name="persons_invited" id="persons_invited" value="{{ old('persons_invited', '') }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('sahra_included') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="sahra_included" id="sahra_included" value="1" required {{ old('sahra_included', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('invite_url') ? 'is-invalid' : '' }}" type="text" name="invite_url" id="invite_url" value="{{ old('invite_url', '') }}">
